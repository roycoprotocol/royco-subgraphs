#!/bin/bash
# Deploy the royco-day-entrypoint subgraph to Goldsky, one network at a time.
#
...
#
# Usage: npm run deploy:subgraphs:entrypoint
...
prepare_and_deploy() {
    local network=$1
    local subgraph_name="royco-day-entrypoint-${network}/${version}"

    echo "Preparing and deploying ${subgraph_name}..."

    npm run "prepare:entrypoint:${network}" \
