...
prepare_and_deploy() {
    local network=$1
    local subgraph_name="royco-rwa-markets-v2-test-${network}/1.0.0" # Note: update version if needed
    
    echo "Preparing and deploying ${subgraph_name}..."

    # preparation command
    mustache config/markets-v2-test/networks/${network}.json config/markets-v2-test/subgraph.template.${network}.yaml > subgraph.yaml && mustache config/markets-v2-test/networks/${network}.json config/markets-v2-test/constants.template.ts > src/constants/static.ts && graph codegen && graph build
