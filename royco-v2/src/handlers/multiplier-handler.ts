...
export function handleMultiplierUpdate(
  ...
  let userMultiplierStateId = generateRawUserMultiplierStateId(
    incentiveCampaignId,
    apAddress
  );
  let rawIncentiveCampaignId =
    generateRawIncentiveCampaignId(incentiveCampaignId);

  let historicalMultiplierState = new RawHistoricalMultiplierState(
    generateRawHistoricalMultiplierStateId(
      incentiveCampaignId,
      apAddress,
      logIndex
  ...
  let userMultiplierState = RawUserMultiplierState.load(
    generateRawUserMultiplierStateId(incentiveCampaignId, apAddress)
  );
...
export function handleAPOfferCreation(entity: APOfferCreated): void {
  let apOffer = new RawMultiplierAPOffer(
    generateRawOfferId(
  ...
  apOffer.rawUserMultiplierStateRefId = generateRawUserMultiplierStateId(
    entity.incentiveCampaignId,
    entity.ap
  );
  apOffer.accountAddress = entity.ap;
  apOffer.offerType = OFFER_TYPE.AP_OFFER;
  apOffer.offerHash = entity.apOfferHash;
...
export function handleAPOfferFill(
  incentiveCampaignId: string,
  apOfferHash: string
): void {
  let apOffer = RawMultiplierAPOffer.load(
    generateRawOfferId(incentiveCampaignId, apOfferHash, OFFER_TYPE.AP_OFFER)
  );
  if (apOffer == null) {
